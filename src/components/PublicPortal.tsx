// Página PÚBLICA do cliente (sem login). Renderizada quando a URL é #/c/<slug>.
// Lê só o documento publicShares/{slug} (que é público) — nunca os dados privados.
import { useEffect, useRef, useState } from 'react';
import { subscribeShare, subscribeComments, recordView, addComment, recordApproval, verifyPin } from '@/services/publicShares';
import type { PublicShare, PublicShareComment, PublicShareCardSnap } from '@/types';

const PRIO_LABEL: Record<string, string> = { high: 'Alta', med: 'Média', low: 'Baixa' };
...
export function PublicPortal({ slug, token = '' }: { slug: string; token?: string }) {
  ...
  useEffect(() => {
    ...
    // Lembra do desbloqueio nesta sessão
  ...
  // Registra visualização (1x) quando o conteúdo fica visível
  useEffect(() => {
...
function Shell({ children, accent }: { children: any; accent: string }) {
  const inApp = typeof window !== 'undefined' && !!(window as any).walkersAPI;
  return (
    <div style={{ position: 'fixed', inset: 0, overflowY: 'auto', background: '#f4f5f9', fontFamily: '-apple-system, Segoe UI, Roboto, sans-serif', color: '#1a1c24', ['--pa' as any]: accent }}>
      {inApp && (
        <button
          ...
          style={{ position: 'fixed', top: 12, left: 12, zIndex: 1000, background: 'rgba(0,0,0,.55)', color: '#fff', border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, cursor: 'pointer' }}
